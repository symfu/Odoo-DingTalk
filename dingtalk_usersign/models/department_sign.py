...
class DingtalkDepartmentSign(models.Model):
    _name = 'dingtalk.department.signs'
    _description = "部门签到记录"
    ...
    line_ids = fields.One2many(comodel_name='dingtalk.department.signs.line', inverse_name='signs_id', string=u'列表')
    ...
    def find_department_sign(self):
        ...
        for res in self:
            res.line_ids = False
            url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'get_dept_checkin')]).value
...
class DingtalkDepartmentSignLine(models.Model):
    _name = 'dingtalk.department.signs.line'
    _description = "部门签到记录列表"
    _rec_name = 'emp_id'

    signs_id = fields.Many2one(comodel_name='dingtalk.department.signs', string=u'签到', ondelete='cascade')
