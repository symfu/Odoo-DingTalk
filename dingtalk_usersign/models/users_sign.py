...
class DingTalkUsersSign(models.Model):
    ...
    line_ids = fields.One2many(comodel_name='dingtalk.users.signs.line', inverse_name='signs_id', string=u'列表')
    ...
    def find_users_sign(self):
        ...
        for res in self:
            res.line_ids = False
            url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'get_user_checkin')]).value
