...
def check_model_send_message(self):
    for res in self:
        if res.id:
            ...
            res_id = res.id
            try:
                check_result = self.env['dingtalk.message.template'].check_message_template(model, 'delete')
            except Exception as e:
                return
            if not check_result:
                return
            logging.info(">>>model:{}-记录id:{},进行发送模板消息".format(model, res_id))
            self.env['dingtalk.message.template'].send_message_template(model, res_id, 'delete')
