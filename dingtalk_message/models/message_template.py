...
class DingtalkMessageTemplateLine(models.Model):
    ...
    template_id = fields.Many2one(comodel_name='dingtalk.message.template', string=u'消息模板', ondelete='cascade')
...
class DingtalkMessageTemplate(models.Model):
    ...
    line_ids = fields.One2many(comodel_name='dingtalk.message.template.line', inverse_name='template_id', string=u'消息字段')
    msg_type = fields.Selection(string=u'接受者', selection=[('00', '员工'), ('01', '部门'), ('03', '所有人')],
                                required=True, default='00')
    emp_ids = fields.Many2many(comodel_name='hr.employee', relation='dingtalk_message_temp_and_employee_rel',
                               column1='template_id', column2='employee_id', string=u'员工',
                               domain=[('din_id', '!=', '')])
    dept_ids = fields.Many2many(comodel_name='hr.department', relation='dingtalk_message_temp_and_department_rel',
    ...
    def check_message_template(self, model, model_type):
        ...
        template = self.env['dingtalk.message.template'].sudo().search([('model_id', '=', model_id)])
        if template:
    ...
    def send_message_template(self, model, res_id, model_type):
        ...
        try:
            if template.msg_type == '03':
                self.env['dingtalk.work.message'].sudo().send_work_message(toall=True, msg=message_dict)
                ...
                for user in template.emp_ids:
                    if user_str == '':
                        ...
                        user_str = user_str + ",{}".format(user.din_id)
                self.env['dingtalk.work.message'].sudo().send_work_message(userstr=user_str, msg=message_dict)
                ...
                for dept in template.dept_ids:
                    if dept_str == '':
                        ...
                        dept_str = dept_str + ",{}".format(dept.din_id)
                self.env['dingtalk.work.message'].sudo().send_work_message(deptstr=dept_str, msg=message_dict)
