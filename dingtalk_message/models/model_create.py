...
def check_model_send_message(self, record):
    ...
    res_id = record.id
    try:
        check_result = self.env['dingtalk.message.template'].check_message_template(model, 'create')
    ...
    self.env['dingtalk.message.template'].send_message_template(model, res_id, 'create')
