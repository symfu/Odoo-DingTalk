...
class DingtalkWorkMessage(models.Model):
    ...
    user_ids = fields.One2many(comodel_name='dingtalk.work.message.user.list', inverse_name='message_id',
                               string=u'接受消息用户列表')
    dep_ids = fields.One2many(comodel_name='dingtalk.work.message.dept.list', inverse_name='message_id',
                              string=u'接受部门消息列表')
    ...
    card_message_ids = fields.One2many(comodel_name='dingtalk.work.card.message.list', inverse_name='message_id',
                                       string=u'按钮列表')
    ...
    oa_message_ids = fields.One2many(comodel_name='dingtalk.work.oa.message.list', inverse_name='message_id',
                                     string=u'OA表单列表')
    ...
    def send_work_message(self, toall=None, userstr=None, deptstr=None, msg=None):
        ...
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'send_work_message')]).value
        token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
        data = {
            'agent_id': self.env['ir.config_parameter'].sudo().get_param('ali_dingtalk.din_agentid'),  # 应用id
            'msg': msg if msg else {}
        }
        if toall:
    ...
    def search_message_state(self):
        ...
        :return:
        """
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'search_work_message')]).value
        token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
        data = {
            'agent_id': self.env['ir.config_parameter'].sudo().get_param('ali_dingtalk.din_agentid'),  # 应用id
    ...
    def search_message_result(self):
        ...
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'work_message_result')]).value
        token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
        data = {
            'agent_id': self.env['ir.config_parameter'].sudo().get_param('ali_dingtalk.din_agentid'),  # 应用id
            'task_id': self.task_id
...
class DingtalkWorkMessageUserList(models.Model):
    ...
    message_id = fields.Many2one(comodel_name='dingtalk.work.message', string=u'消息', ondelete='cascade')
...
class CardMessageList(models.Model):
    _name = 'dingtalk.work.card.message.list'
    _description = '卡片消息列表'
    _rec_name = 'title'
...
class OaMessageList(models.Model):
    _name = 'dingtalk.work.oa.message.list'
