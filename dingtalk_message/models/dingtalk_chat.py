...
class DingTalkChat(models.Model):
    ...
    def create_dingtalk_chat(self):
        ...
        for res in self:
            ...
            url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'chat_create')]).value
    ...
    def write_dingtalk_chat(self):
        ...
        for res in self:
            ...
            url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'chat_update')]).value
            token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
            data = {
                'chatid': res.chat_id,
                'name': res.name,
    ...
    @api.model
    def process_dingtalk_chat_onchange(self, msg):
        """
        处理回调
        :param msg: msg
        :return:
        """
        print(msg)
        chat = self.env['dingtalk.chat'].sudo().search([('chat_id', '=', msg.get('ChatId'))])
        # 群会话更换群主
...
class DingTalkChatUserModelAdd(models.TransientModel):
    ...
    def default_get(self, fields):
        res = super(DingTalkChatUserModelAdd, self).default_get(fields)
        chat_id = self.env.context.get('active_id', False)
        ding_chat = self.env['dingtalk.chat'].browse(chat_id)
...
class DingTalkChatUserModelDel(models.TransientModel):
    ...
    @api.model
    def default_get(self, fields):
        res = super(DingTalkChatUserModelDel, self).default_get(fields)
        chat_id = self.env.context.get('active_id', False)
        ding_chat = self.env['dingtalk.chat'].browse(chat_id)
...
class DingTalkSendChatMessage(models.TransientModel):
    ...
    def send_dingtalk_message(self):
        ...
        chat_id = self.env.context.get('active_id', False)
        ding_chat = self.env['dingtalk.chat'].browse(chat_id)
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'chat_send')]).value
        token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
        data = {
            'chatid': ding_chat.chat_id,
            'msg': {
...
class DingTalkChatList(models.TransientModel):
    _name = 'get.dingtalk.chat.list'
    ...
    def get_chat_info(self):
        ...
        for res in self:
            ...
            url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'chat_get')]).value
            token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
            data = {
                'chatid': res.chat_id,
            }
            try:
                ...
                if result.get('errcode') == 0:
                    ...
                    chat = self.env['dingtalk.chat'].sudo().search([('chat_id', '=', res.chat_id)])
                    if chat:
                        chat.write(data)
                    else:
                        self.env['dingtalk.chat'].sudo().create(data)
                else:
