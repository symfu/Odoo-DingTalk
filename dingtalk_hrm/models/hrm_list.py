...
class DingtalkHrmList(models.Model):
    ...
    line_ids = fields.One2many(comodel_name='dingtalk.hrm.list.line', inverse_name='list_is', string=u'信息列表')
...
class DingtalkHrmListline(models.Model):
    ...
    list_is = fields.Many2one(comodel_name='dingtalk.hrm.list', string=u'获取员工花名册', ondelete='cascade')
...
class GetDingtalkHrmList(models.TransientModel):
    ...
    emp_ids = fields.Many2many(comodel_name='hr.employee', relation='dingtalk_hrm_list_and_hr_employee_rel',
    ...
    def get_hrm_list(self):
        ...
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'hrm_list')]).value
        ...
        try:
            ...
            if result.get('errcode') == 0:
                for res in result.get('result'):
                    ...
                    if emp:
                        hrm = self.env['dingtalk.hrm.list'].search([('emp_id', '=', emp[0].id)])
                        if hrm:
                            hrm.write({
                            ...
                            self.env['dingtalk.hrm.list'].sudo().create({
                                'emp_id': emp[0].id,
                                'department_id': emp[0].department_id.id,
                                'line_ids': line_list
                            })
        ...
        action = self.env.ref('dingtalk_hrm.dingtalk_hrm_list_action')
