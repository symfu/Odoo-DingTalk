...
class DingTalkSimpleGroups(models.Model):
    ...
    manager_list = fields.Many2many(comodel_name='hr.employee', relation='dingtalk_simple_group_hr_emp_rel',
                                    column1='group_id', column2='emp_id', string=u'负责人')
    dept_name_list = fields.Many2many(comodel_name='hr.department', relation='dingtalk_simple_group_hr_dept_rel',
    ...
    def get_simple_groups(self):
        ...
        """
        logging.info(">>>获取考勤组...")
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'attendance_getsimplegroups')]).value
        ...
        for emp in emps:
            ...
            try:
                ...
                if result.get('errcode') == 0:
                    ...
                    groups = self.env['dingtalk.simple.groups'].sudo().search([('group_id', '=', res.get('group_id'))])
                    if groups:
                        self._cr.execute(
