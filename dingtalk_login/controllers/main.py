...
class DingTalkLogin(Home, http.Controller):
    ...
    def web_dingtalk_login(self, *args, **kw):
        ...
        return request.render('dingtalk_login.signup', values)

    @http.route('/dingtalk_login/get_url', type='http', auth="none")
    def get_url(self, **kw):
        """
        拼接访问钉钉的验证用户的url
        :param kw:
        :return:
        """
        url = request.env['ali.dingtalk.system.conf'].sudo().search([('key', '=', 'sns_authorize')]).value
        login_appid = request.env['ir.config_parameter'].sudo().get_param('ali_dingtalk.din_login_appid')
        # 获取传递过来当前的url和端口信息
    ...
    def getUserInfobyDincode(self, d_code):
        ...
        :return:
        """
        url = request.env['ali.dingtalk.system.conf'].sudo().search([('key', '=', 'getuserinfo_bycode')]).value
        login_appid = request.env['ir.config_parameter'].sudo().get_param('ali_dingtalk.din_login_appid')
        key = request.env['ir.config_parameter'].sudo().get_param('ali_dingtalk.din_login_appsecret')
    ...
    def getUseridByUnionid(self, unionid):
        """根据unionid获取userid"""
        url = request.env['ali.dingtalk.system.conf'].sudo().search([('key', '=', 'getUseridByUnionid')]).value
    ...
    def _do_err_redirect(self, errmsg):
        ...
        return request.render('dingtalk_login.signup', err_values)

    @http.route('/web/dingtalk/account_login', type='http', auth='public', website=True, sitemap=False)
    def web_dingtalk_account_login(self, *args, **kw):
        """
        ...
        """
        url = request.env['ali.dingtalk.system.conf'].sudo().search([('key', '=', 'sns_authorize')]).value
        appid = request.env['ir.config_parameter'].sudo().get_param('ali_dingtalk.din_login_appid')
        redirect_url = '{}web/dingtalk/account_action_login'.format(request.httprequest.host_url)
    ...
    @http.route('/web/dingtalk/account_action_login', type='http', auth="none")
    def dingtalk_account_action_login(self, redirect=None, **kw):
        """
        :param redirect:
        :param kw:
        :return:
