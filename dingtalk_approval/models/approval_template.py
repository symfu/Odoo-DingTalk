...
class DingTalkApprovalTemplate(models.Model):
    ...
    def get_template(self):
        """获取审批模板"""
        logging.info(">>>获取审批模板...")
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'process_listbyuserid')]).value
        ...
        try:
            ...
            if result.get('errcode') == 0:
                ...
                for process in d_res.get('process_list'):
                    ...
                    template = self.env['dingtalk.approval.template'].search(
                        [('process_code', '=', process.get('process_code'))])
                    if template:
                        template.write(data)
                    else:
    ...
    def get_get_template_number_by_user(self):
        ...
        if emp and emp.din_id:
            url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'process_gettodonum')]).value
            token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
            data = {
                'userid': emp.din_id,
            }
    ...
    def get_processinstance(self, pid, pcode):
        ...
        :return:
        """
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'processinstance_get')]).value
        token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
        data = {
        ...
        try:
            ...
            if result.get('errcode') == 0:
                ...
                temp = self.env['dingtalk.approval.template'].sudo().search([('process_code', '=', pcode)])
                if temp:
                    appro = self.env['dingtalk.approval.control'].sudo().search([('template_id', '=', temp[0].id)])
                    if appro:
