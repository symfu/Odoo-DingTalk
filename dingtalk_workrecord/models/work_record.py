...
class DingTalkWorkRecord(models.Model):
    ...
    line_ids = fields.One2many(comodel_name='dingtalk.work.record.list', inverse_name='record_id', string=u'待办事项表单')
    ...
    def send_record(self):
        ...
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'workrecord_add')]).value
                ...
                try:
                    if 'list' in result:
                        for res in result.get('list'):
                            record = self.env['dingtalk.work.record'].search(
                            ...
                            if not record:
                                self.env['dingtalk.work.record'].create(data)
    ...
    def get_workrecord_url(self, user_id, offset, limit):
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'workrecord_getbyuserid')]).value
        token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
        data = {
            'userid': user_id,
            'offset': offset,
    ...
    def record_update(self):
        ...
        for res in self:
            logging.info("待办更新")
            url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'workrecord_update')]).value
            token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
            data = {
    ...
    def get_record_number(self):
        ...
        if emp:
            record = self.env['dingtalk.work.record'].sudo().search(
...
class DingTalkWorkRecordList(models.Model):
    ...
    record_id = fields.Many2one(comodel_name='dingtalk.work.record', string=u'待办', ondelete='cascade')
