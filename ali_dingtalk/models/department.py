...
class HrDepartment(models.Model):
    ...
    def create_ding_department(self):
        for res in self:
            ...
            url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'department_create')]).value
    ...
    def update_ding_department(self):
        for res in self:
            url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'department_update')]).value
            token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
            # 获取部门din_id
    ...
    def delete_din_department(self, din_id):
        """删除钉钉部门"""
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'department_delete')]).value
        token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
        data = {
            'id': din_id,  # userid
        }
    ...
    def synchronous_dingtalk_department(self):
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'department_list')]).value
        token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
        data = {'id': 1}
        result = requests.get(url="{}{}".format(url, token), params=data, timeout=15)
