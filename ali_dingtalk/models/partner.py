...
class ResPartner(models.Model):
    ...
    def create_ding_partner(self):
        for res in self:
            ...
            url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'extcontact_create')]).value
    ...
    def update_ding_partner(self):
        """修改员工时同步至钉钉"""
        for res in self:
            url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'extcontact_update')]).value
            token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
            # 获取标签
            label_list = list()
            if res.category_id:
    ...
    def unlink(self):
        for res in self:
            ...
            if self.env['ir.config_parameter'].sudo().get_param('ali_dingtalk.din_delete_extcontact'):
    ...
    def delete_din_extcontact(self, din_userid):
        """删除钉钉联系人"""
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'extcontact_delete')]).value
        token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
        data = {
    ...
    @api.model
    def synchronous_dingtalk_res_partner(self):
        """
        在联系人看板、列表中点击同步时执行同步方法
        :return:
        """
        # 同步联系人标签
        result = self.synchronous_dingtalk_category()
        if not result.get('state'):
            return result
        # 同步联系人
        return self.synchronous_dingtalk_partner()

    @api.model
    def synchronous_dingtalk_category(self):
        logging.info(">>>同步钉钉联系人标签start")
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'listlabelgroups')]).value
        token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
        headers = {'Content-Type': 'application/json'}
        data = {
            'size': 100,
    ...
    @api.model
    def synchronous_dingtalk_partner(self):
        logging.info(">>>同步钉钉联系人列表start")
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'extcontact_list')]).value
        token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
        headers = {'Content-Type': 'application/json'}
        data = {
            'size': 100,
