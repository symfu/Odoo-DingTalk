odoo.define('dingtalk.res.department.synchronous.button', function (require) {
    "use strict";
    ...
    let start_synchronous_department = function () {
        rpc.query({
            model: 'hr.department',
            method: 'synchronous_dingtalk_department',
            args: [],
    ...
    ListController.include({
        renderButtons: function ($node) {
            ...
            if (tree_model == 'hr.department') {
                ...
                let button2 = $(but).click(this.proxy('synchronous_dingtalk_department_list'));
                this.$buttons.append(button2);
            }
            return $buttons;
        },
        synchronous_dingtalk_department_list: function () {
            new Dialog(this, {
                title: "同步钉钉部门",
                size: 'medium',
                buttons: [
    ...
    KanbanController.include({
        renderButtons: function ($node) {
            ...
            if (tree_model == 'hr.department') {
                let but = "<button type=\"button\" t-if=\"widget.modelName == 'hr.department'\" class=\"btn btn-primary\">同步钉钉部门</button>";
                let button2 = $(but).click(this.proxy('synchronous_dingtalk_department_kanban'));
                this.$buttons.append(button2);
            }
            return $buttons;
        },
        synchronous_dingtalk_department_kanban: function () {
            new Dialog(this, {
                title: "同步钉钉部门",
                size: 'medium',
                buttons: [
