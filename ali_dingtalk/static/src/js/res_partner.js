odoo.define('dingtalk.res.partner.synchronous.button', function (require) {
    "use strict";
    ...
    let start_synchronous_partner = function () {
        rpc.query({
            model: 'res.partner',
            method: 'synchronous_dingtalk_res_partner',
            args: [],
    ...
    ListController.include({
        renderButtons: function ($node) {
            ...
            if (tree_model == 'res.partner') {
                ...
                let button2 = $(but).click(this.proxy('synchronous_dingtalk_partner_list'));
                this.$buttons.append(button2);
            }
            return $buttons;
        },
        synchronous_dingtalk_partner_list: function () {
            new Dialog(this, {
                title: "同步钉钉联系人",
                size: 'medium',
                buttons: [
    ...
    KanbanController.include({
        renderButtons: function ($node) {
            ...
            if (tree_model == 'res.partner') {
                let but = "<button type=\"button\" t-if=\"widget.modelName == 'res.partner'\" class=\"btn btn-primary\">同步钉钉联系人</button>";
                let button2 = $(but).click(this.proxy('synchronous_dingtalk_partner_kanban'));
                this.$buttons.append(button2);
            }
            return $buttons;
        },
        synchronous_dingtalk_partner_kanban: function () {
            new Dialog(this, {
                title: "同步钉钉联系人",
                size: 'medium',
                buttons: [
