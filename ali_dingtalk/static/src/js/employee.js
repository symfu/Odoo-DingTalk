odoo.define('dingtalk.res.employee.synchronous.button', function (require) {
    "use strict";
    ...
    let start_synchronous_employee = function () {
        rpc.query({
            model: 'hr.employee',
            method: 'synchronous_dingtalk_employee',
            args: [],
    ...
    ListController.include({
        renderButtons: function ($node) {
            ...
            if (tree_model == 'hr.employee') {
                ...
                let button2 = $(but).click(this.proxy('synchronous_dingtalk_employee_list'));
                this.$buttons.append(button2);
            }
            return $buttons;
        },
        synchronous_dingtalk_employee_list: function () {
            new Dialog(this, {
                title: "同步钉钉员工",
                size: 'medium',
                buttons: [
    ...
    KanbanController.include({
        renderButtons: function ($node) {
            ...
            if (tree_model == 'hr.employee') {
                let but = "<button type=\"button\" t-if=\"widget.modelName == 'hr.employee'\" class=\"btn btn-primary\">同步钉钉员工</button>";
                let button2 = $(but).click(this.proxy('synchronous_dingtalk_employee_kanban'));
                this.$buttons.append(button2);
            }
            return $buttons;
        },
        synchronous_dingtalk_employee_kanban: function () {
            new Dialog(this, {
                title: "同步钉钉员工",
                size: 'medium',
                buttons: [
