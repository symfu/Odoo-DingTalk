...
- **考勤**  (v1.4)
  ...
  >
  > 2.打卡列表：
  > 获取钉钉人员打卡信息，手动进行获取，每次只能获取7天的打卡信息
  >
- **签到**  (v1.3)
  > 1.签到记录：
  > 获取钉钉相对应的签到记录
...
- **手动同步基础信息**   (v1.1)
  >
  >在对应的联系人、员工、部门看板视图或列表视图中可点击同步按钮，获取钉钉中的数据，也可将odoo系统中的数据上传至钉钉中，目前仅支持单个上传，批量上传将   >会在下个更新版本中体现
  >
...
### 常见问题：
- **安装odooDingTalk或者升级其中模块时候请注意**
  >
  > 1.一定要通过主模块ali_dingtalk进行安装、升级，以免出现错误！
  >
  >
...
- **仪表盘：获取公告失败,详情为:无效的USERID、代审批数、公告数**
  ...
  >
- **获取考勤组成员的时候提示： 考勤组有更新,请先获取最新的考勤组!**
  >
  > 那就点击 获取考勤组成员 旁的 获取考勤组即可
  >
- **考勤组成员列表无法更新**
  >
...
- **若看不到的请检查权限**
  ...
  >
  > 解决办法是： 将dingtalk_callback模块中的file文件夹下的http.py文件 覆盖/替换到`odoo-root/odoo/http.py`文件
  >
  > Odoo-root: 为odoo源码目录
  >
- **关于通讯录管理(员工、部门、联系人)**
  >
  > 获取用户时候，请先获取部门，再获取用户不然会报错
  >
  > 获取用户以后记得和odoo系统中相关账号进行绑定操作（用户-HR设置关联）
  >
