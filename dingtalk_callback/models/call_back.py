...
class DingTalkCallback(models.Model):
    ...
    call_id = fields.Many2one(comodel_name='dingtalk.users.callback.list', string=u'回调类型', ondelete='cascade')
    ...
    call_ids = fields.Many2many(comodel_name='dingtalk.users.callback.list', relation='dingtalk_users_callback_and_list_ref',
    ...
    def onchange_value_type(self):
        if self.value_type:
            ...
            if self.value_type == 'all':
                for li in self.env['dingtalk.users.callback.list'].sudo().search([]):
                    call_ids.append(li.id)
            else:
                for li in self.env['dingtalk.users.callback.list'].sudo().search([('value_type', '=', self.value_type)]):
                    call_ids.append(li.id)
    ...
    def register_call_back(self):
        ...
        logging.info(">>>注册事件...")
        for res in self:
            url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'register_call_back')]).value
    ...
    def update_call_back(self):
        ...
        """
        for res in self:
            url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'update_call_back')]).value
            token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
            call_list = list()
            for call in res.call_ids:
                call_list.append(call.value)
    ...
    def delete_call_back(self, call_token):
        logging.info(">>>删除事件...")
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'delete_call_back')]).value
        token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
        data = {
    ...
    def get_all_call_back(self):
        ...
        :return:
        """
        url = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'get_call_back')]).value
        token = self.env['ali.dingtalk.system.conf'].search([('key', '=', 'token')]).value
        try:
            ...
            if result.get('errcode') != 0:
                ...
                for tag in result.get('call_back_tag'):
                    callback_list = self.env['dingtalk.users.callback.list'].search([('value', '=', tag)])
                    if callback_list:
                        tag_list.append(callback_list[0].id)
                callback = self.env['dingtalk.users.callback'].search([('company_id', '=', self.env.user.company_id.id)])
                data = {
                ...
                if callback:
                    ...
                    self.env['dingtalk.users.callback'].create(data)
                return {'state': True}
        except ReadTimeout:
