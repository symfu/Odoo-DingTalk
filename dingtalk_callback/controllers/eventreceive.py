...
class CallBack(Home, http.Controller):
    ...
    def callback_users(self, **kw):
        ...
        if event_type == 'user_add_org' or event_type == 'user_modify_org' or event_type == 'user_leave_org':
            if event_type == 'user_leave_org':
                ...
                request.env['hr.employee'].sudo().synchronous_dingtalk_employee()
            ...
            if event_type == 'org_dept_remove':
                ...
                request.env['hr.department'].sudo().synchronous_dingtalk_department()
        # -----员工角色-------
        ...
        # -----用户签到-----------
            ...
            request.env['dingtalk.signs.list'].sudo().get_signs_by_user(msg.get('StaffId'), msg.get('TimeStamp'))
        # -------群会话事件----------
    ...
    def get_bash_attr(self):
        ...
        :return:
        """
        call_back = request.env['dingtalk.users.callback'].sudo().search([])
        if not call_back:
            raise UserError("钉钉回调管理单据错误，无法获取token和encode_aes_key值!")
        din_corpId = request.env['ir.config_parameter'].sudo().get_param('ali_dingtalk.din_corpId')
        if not din_corpId:
    ...
    def bpms_instance_change(self, msg):
        ...
        if temp:
            appro = request.env['dingtalk.approval.control'].sudo().search([('template_id', '=', temp[0].id)])
            if appro:
